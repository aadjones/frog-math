...
export function setupTouchScroll(
  ...
  const handleTouchStart = (e: TouchEvent) => {
    if (e.touches.length === 1) {
      ...
      state.touchStartCamX = state.hasManualPosition ? state.manualCamX : callbacks.getCurrentCamX();
...
export function getCameraX(
  ...
  return (state.isDragging || state.hasManualPosition) ? state.manualCamX : defaultCamX;
}
...
export function getSimpleCameraX(
  ...
  const alpha = animating ? Math.max(0, Math.min(1, (t - hopStart) / hopDur)) : 1;
